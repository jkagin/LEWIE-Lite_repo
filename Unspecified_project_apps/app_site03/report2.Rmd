...
\newpage
\subsection{Local Economy Impacts of Local Government Spending (US\$)}

Local government spends taxes on local activities, which also creates multipliers.

How much in additional local government spending ($) do you want to simulate?
```{r echo=FALSE, comment=NA}
  cat(params$sim_LocalGSpending)
```

&nbsp;

### EFFECTS OF THIS INCREASE IN LOCAL GOVERNMENT SPENDING ON...
```{r echo=FALSE, out.width="50%", fig.align="default"}
  params$inc10()
  params$linc10()
```
